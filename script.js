...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🍁 טריוויה קנדית - אתחול התחיל');
    
    loadQuestions().then(() => {
        console.log('✅ שאלות נטענו, מאתחל משחק');
        initializeGame();
    }).catch(error => {
        console.warn('⚠️ לא ניתן לטעון את questions.json, משתמש בשאלות מובנות:', error.message);
        // שימוש בשאלות מובנות כ-fallback
        initializeGame();
    });
    
    // וידוא שכפתור ההוראות קיים
    setTimeout(() => {
        const instructionsBtn = document.getElementById('showInstructions');
        const instructionsModal = document.getElementById('instructionsModal');
        console.log('🔍 בדיקת אלמנטים:');
        console.log('כפתור הוראות:', instructionsBtn ? '✅ קיים' : '❌ לא נמצא');
        console.log('מודל הוראות:', instructionsModal ? '✅ קיים' : '❌ לא נמצא');
        
        if (instructionsBtn) {
            console.log('📋 כפתור הוראות זמין לשימוש');
        }
    }, 100);
});
...
function setupEventListeners() {
    // בדיקה שהאלמנטים קיימים לפני הוספת listeners
    ...
    if (showInstructionsBtn) {
        showInstructionsBtn.addEventListener('click', function() {
            console.log('כפתור הוראות נלחץ!'); // debug
            showInstructions();
        });
    }
    if (closeInstructionsBtn) closeInstructionsBtn.addEventListener('click', hideInstructions);
    if (startPlayingBtn) startPlayingBtn.addEventListener('click', hideInstructions);
    ...
    console.log('Event listeners הוגדרו. כפתור הוראות:', showInstructionsBtn ? 'נמצא' : 'לא נמצא');
}
...
function showInstructions() {
    console.log('פונקציית showInstructions התחילה');
    const modal = document.getElementById('instructionsModal');
    if (modal) {
        modal.style.display = 'flex';
        console.log('מודל הוראות הוצג');
    } else {
        console.error('מודל הוראות לא נמצא!');
    }
}

function hideInstructions() {
    console.log('פונקציית hideInstructions התחילה');
    const modal = document.getElementById('instructionsModal');
    if (modal) {
        modal.style.display = 'none';
        console.log('מודל הוראות הוסתר');
    } else {
        console.error('מודל הוראות לא נמצא!');
    }
}

// פונקציה גלובלית לשימוש מה-HTML
window.showInstructionsNow = function() {
    console.log('פונקציה גלובלית לhוראות נקראה');
    showInstructions();
};

window.hideInstructionsNow = function() {
    console.log('פונקציה גלובלית לסגירת הוראות נקראה');
    hideInstructions();
};
